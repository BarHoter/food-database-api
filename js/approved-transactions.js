document.addEventListener('DOMContentLoaded', function() {
    ...
    loadApprovedTransactions(role);
});

function loadApprovedTransactions(role) {
    ...
    // Filter approved transactions based on role
    const approvedTransactions = transactions.filter(t => {
        if (role === 'buyer') {
            return t.BuyerID === currentUser && t.Status === 'Approved';
        } else {
            return t.SellerID === currentUser && t.Status === 'Approved';
        }
    });

    const container = document.getElementById('approvedTransactions');
    
    if (approvedTransactions.length === 0) {
        container.innerHTML = '<p class="no-data">No approved transactions found</p>';
        return;
    }
